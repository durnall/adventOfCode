...
var inputs = @"QLFdFCdlLcVqdvFLnFLSSShZwptfHHhfZZZpSwfmHp
...
var puzzleInputs = inputs.Split("\n");

var total = 0;

foreach (var puzzleInput in puzzleInputs)
{
    var first = puzzleInput.Substring(0, puzzleInput.Length / 2).ToArray();
    var second = puzzleInput.Substring(puzzleInput.Length / 2, puzzleInput.Length / 2).ToArray();
    ...
    foreach (var match in matches)
    {
        // a-z = 1-26
        // A-Z = 27-52
        var priorityLookup = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
        total += priorityLookup.IndexOf(match) + 1;
    }
}
...
puzzleInputs = inputs.Split("\r\n");

total = 0;

for (int i = 0; i < puzzleInputs.Length / 3; i++)
{
    var pos = i * 3;
    var first = puzzleInputs[pos].ToArray();
    var second = puzzleInputs[pos + 1].ToArray();
    var third = puzzleInputs[pos + 2].ToArray();

    var matches = first.Intersect(second.Intersect(third));
    foreach (var match in matches)
    {
        // a-z = 1-26
        // A-Z = 27-52
